...
var GWeb = function(config) {
          ...
          for ( var key in _super.config[sheet].params[i]) {
            if (key == '_input') {
              ...
              if (typeof _input === 'string') {
                _super.config[sheet].params[i]._input = JSON.parse(_input);
              }
              for ( var key in _input) {
                if (key.indexOf('_') == -1) {
                  if (typeof _input[key] == 'number') {
                    ...
                    _super.config[sheet].params[i]._input[key] = $('#' + key)
                        .val();
                  }
                }
              }
              ...
              if (key.indexOf('_') != 0) {
                if ($('#' + col).val()) {
                  _super.config[sheet].params[i][key] = $('#' + col).val();
                }
              }
            }
          }
  ...
  _super.getRowIndex = function(data, colIdx, val) {
    var i = 0;
  ...
  _super.getNextSn = function(data, colIdx) {
    return data[data.length - 1][colIdx] + 1;
  }
