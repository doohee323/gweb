...
var GWeb = function(config) {
	...
	_super.getGAuth = function(sheet) {
		if (!_super.config[sheet]) {
			console.log('_super.config[' + sheet + '] undefined!');
			return false;
		} else if (_super.config[sheet].gauth) {
	...
	_super.execute = function(sheet, option, cb) {
		...
		if (_super.config[sheet].input) {
			...
			if(_super._docId) {
				params.input._docId = _super._docId;
			}
		} else {
			for (var i = 0; i < _super.config[sheet].params.length; i++) {
			...
			if(_super._docId) {
				params.params._docId = _super._docId;
			}
		}
		if (option.owner) {
			if (params.input) {
